<template>
  <div class="header">
    ...
    <div class="nav-links">
      ...
      <!-- 题目模块下拉菜单 -->
      <div class="dropdown" :class="{ open: activeDropdown === 'exam' }" @mouseenter="showDropdown('exam')" @mouseleave="hideDropdown">
        <button class="nav-link dropdown-btn">
      ...
      <!-- 音视频资源模块下拉菜单 -->
      <div class="dropdown" :class="{ open: activeDropdown === 'media' }" @mouseenter="showDropdown('media')" @mouseleave="hideDropdown">
        <button class="nav-link dropdown-btn">
          音视频模块 <span class="dropdown-indicator"></span>
        </button>
...
.header {
  ...
  padding: 0 30px;
  height: 60px;
  background: linear-gradient(90deg, #66bb6a 0%, #5ca660 100%);
...
.dropdown-indicator {
  width: 10px;
  height: 10px;
  border-left: 2px solid rgba(249, 255, 249, 0.9);
  border-bottom: 2px solid rgba(249, 255, 249, 0.9);
